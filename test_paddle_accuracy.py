...
def process_directory_with_tqdm(directory_path, min_conf=0, output_file=None, num_threads=1, max_images=None, batch_size=50):
    ...
    checkpoint_file = f"test_output/checkpoint_paddle_conf{min_conf}.json"
...
def main():
    ...
    for conf in tqdm(conf_thresholds, desc="Testing confidence thresholds", unit="level"):
        ...
        try:
            ...
            conf_output = f"test_output/paddle_fuzzy_results_conf{conf}.json"
            ...
            # Evaluate results
            start_time = time.time()
            eval_output = f"test_output/paddle_fuzzy_eval_results_conf{conf}.json"
    ...
    try:
        with open("test_output/paddle_fuzzy_accuracy.json", "w") as f:
            json.dump(results, f, indent=2)
    except Exception as e:
