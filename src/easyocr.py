...
def detect_text(image_path, min_conf=0):
    """
    Detect and localize text in an image using EasyOCR.
    ...
    # Initialize the EasyOCR reader with CPU mode
    reader = easyocr.Reader(['en'], gpu=False)
    
    # Use EasyOCR to detect text in the input image
    ...
    for (bbox, text, prob) in results:
        # Convert confidence from 0-1 scale to 0-100 for consistency with Tesseract
        conf = int(prob * 100)
...
def get_text_boxes(image_path, min_conf=0):
    ...
    # Initialize EasyOCR reader with CPU mode
    reader = easyocr.Reader(['en'], gpu=False)
    ...
    try:
        ...
        # Process results
        for (bbox, text, prob) in results:
            # Convert confidence from 0-1 scale to match expected format
            conf = prob * 100
            
            # Only consider results if they have text and meet minimum confidence
            if conf > min_conf and text.strip():
                # Extract bounding box coordinates
                # EasyOCR returns points in order: top-left, top-right, bottom-right, bottom-left
                # Convert to [x, y, w, h] format
                x = int(min(bbox[0][0], bbox[3][0]))
                y = int(min(bbox[0][1], bbox[1][1]))
                w = int(max(bbox[1][0] - bbox[0][0], bbox[2][0] - bbox[3][0]))
                h = int(max(bbox[3][1] - bbox[0][1], bbox[2][1] - bbox[1][1]))
                ...
                if w > 0 and h > 0:
                    ...
                    texts.append(text)
                    confidences.append(prob)  # Already in [0, 1] range
