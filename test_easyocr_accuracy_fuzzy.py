...
def process_directory_with_tqdm(directory_path, min_conf=0, output_file=None, num_threads=1, max_images=None, batch_size=50):
    ...
    problematic_images_file = "test_output/problematic_images_easyocr_fuzzy.txt"
...
def main():
    print("Starting EasyOCR evaluation with fuzzy matching...")
    
    # Parse command line arguments
    import argparse
    parser = argparse.ArgumentParser(description='Evaluate EasyOCR accuracy with fuzzy matching')
    ...
    # Test EasyOCR
    print("\n===== Evaluating EasyOCR with fuzzy matching (all images in 'images' folder) =====")
    for conf in tqdm(conf_thresholds, desc="Testing confidence thresholds", unit="level"):
        ...
        try:
            ...
            conf_output = f"test_output/easyocr_fuzzy_results_conf{conf}.json"
            ...
            # Evaluate results
            start_time = time.time()
            eval_output = f"test_output/easyocr_fuzzy_eval_results_conf{conf}.json"
    ...
    try:
        with open("test_output/easyocr_fuzzy_accuracy.json", "w") as f:
            json.dump(results, f, indent=2)
    except Exception as e:
        print(f"Error saving summary results: {e}")
    
    # Print final summary
    print("\n=== SUMMARY: EasyOCR with Fuzzy Matching ===")
    ...
    if results:
        try:
            ...
            print(f"\n=== Best EasyOCR Result with Fuzzy Matching ===")
